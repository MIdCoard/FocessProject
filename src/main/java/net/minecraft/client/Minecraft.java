...
public class Minecraft implements IThreadListener, ISnooperInfo {
	...
	public Minecraft(final GameConfiguration gameConfig) {
		...
		Minecraft.LOGGER.debug("(Session ID is {})", this.session.getSessionID());
		...
		Focess.loadDefault();
		Focess.setUsername(this.session.getUsername());
	}
	...
	public CrashReport addGraphicsAndWorldToCrashReport(final CrashReport theCrash) {
		...
		theCrash.getCategory().setDetail("OpenGL", () -> GlStateManager.glGetString(7937) + " GL version "
				+ GlStateManager.glGetString(7938) + ", " + GlStateManager.glGetString(7936));
		...
		theCrash.getCategory().setDetail("Current Language",
				() -> Minecraft.this.mcLanguageManager.getCurrentLanguage().toString());
		theCrash.getCategory().setDetail("Profiler Position",
	...
	private void checkGLError(final String message) {
		...
		if (i != 0) {
			...
			Minecraft.LOGGER.error("{}: {}", Integer.valueOf(i), s);
		}
	}
	...
	public void displayCrashReport(final CrashReport crashReportIn) {
		final File file1 = new File(Minecraft.getMinecraft().mcDataDir, "crash-reports");
	...
	private void displayDebugInfo(final long elapsedTicksTime) {
		if (this.mcProfiler.profilingEnabled) {
			...
			for (int l = 0; l < list.size(); ++l) {
				...
				final int i1 = MathHelper.floor(profiler$result1.usePercentage / 4.0D) + 1;
				...
				for (int j2 = i1; j2 >= 0; --j2) {
					final float f = (float) ((d0 + profiler$result1.usePercentage * j2 / i1) * (Math.PI * 2D) / 100.0D);
					final float f1 = MathHelper.sin(f) * 160.0F;
					final float f2 = MathHelper.cos(f) * 160.0F * 0.5F;
				...
				for (int i3 = i1; i3 >= 0; --i3) {
					final float f3 = (float) ((d0 + profiler$result1.usePercentage * i3 / i1) * (Math.PI * 2D)
							/ 100.0D);
					final float f4 = MathHelper.sin(f3) * 160.0F;
					final float f5 = MathHelper.cos(f3) * 160.0F * 0.5F;
			...
			this.fontRendererObj.drawStringWithShadow(s, j + 160 - this.fontRendererObj.getStringWidth(s), k - 80 - 16,
					16777215);
			...
			for (int k2 = 0; k2 < list.size(); ++k2) {
				...
				this.fontRendererObj.drawStringWithShadow(s1, j - 160, k + 80 + k2 * 8 + 20,
						profiler$result2.getColor());
				s1 = decimalformat.format(profiler$result2.usePercentage) + "%";
				this.fontRendererObj.drawStringWithShadow(s1, j + 160 - 50 - this.fontRendererObj.getStringWidth(s1),
						k + 80 + k2 * 8 + 20, profiler$result2.getColor());
				s1 = decimalformat.format(profiler$result2.totalUsePercentage) + "%";
				this.fontRendererObj.drawStringWithShadow(s1, j + 160 - this.fontRendererObj.getStringWidth(s1),
						k + 80 + k2 * 8 + 20, profiler$result2.getColor());
			}
		}
	}
	 ...
	 */
	public void draw(final int posX, final int posY, final int texU, final int texV, final int width, final int height,
			final int red, final int green, final int blue, final int alpha) {
		final BufferBuilder bufferbuilder = Tessellator.getInstance().getBuffer();
		bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
		bufferbuilder.pos(posX, posY + height, 0.0D).tex(texU * 0.00390625F, (texV + height) * 0.00390625F)
				.color(red, green, blue, alpha).endVertex();
		bufferbuilder.pos(posX + width, posY + height, 0.0D)
				.tex((texU + width) * 0.00390625F, (texV + height) * 0.00390625F).color(red, green, blue, alpha)
				.endVertex();
		bufferbuilder.pos(posX + width, posY, 0.0D).tex((texU + width) * 0.00390625F, texV * 0.00390625F)
				.color(red, green, blue, alpha).endVertex();
		bufferbuilder.pos(posX, posY, 0.0D).tex(texU * 0.00390625F, texV * 0.00390625F).color(red, green, blue, alpha)
				.endVertex();
	...
	private void drawSplashScreen(final TextureManager textureManagerInstance) throws LWJGLException {
		...
		bufferbuilder.pos(this.displayWidth, this.displayHeight, 0.0D).tex(0.0D, 0.0D).color(255, 255, 255, 255)
				.endVertex();
	...
	private void func_193986_ar() {
		final SearchTree<ItemStack> searchtree = new SearchTree<>((p_193988_0_) -> {
			return (List<String>) p_193988_0_.getTooltip((EntityPlayer) null, ITooltipFlag.TooltipFlags.NORMAL).stream()
		...
		final NonNullList<ItemStack> nonnulllist = NonNullList.<ItemStack>func_191196_a();
		...
		final SearchTree<RecipeList> searchtree1 = new SearchTree<>((p_193990_0_) -> {
	 ...
	 */
	public void launchIntegratedServer(final String folderName, final String worldName,
			@Nullable WorldSettings worldSettingsIn) {
		this.loadWorld((WorldClient) null);
		System.gc();
		...
		try {
			final YggdrasilAuthenticationService yggdrasilauthenticationservice = new YggdrasilAuthenticationService(
					this.proxy, UUID.randomUUID().toString());
			final MinecraftSessionService minecraftsessionservice = yggdrasilauthenticationservice
					.createMinecraftSessionService();
			final GameProfileRepository gameprofilerepository = yggdrasilauthenticationservice
					.createProfileRepository();
			final PlayerProfileCache playerprofilecache = new PlayerProfileCache(gameprofilerepository,
	...
	public void loadWorld(@Nullable final WorldClient worldClientIn, final String loadingMessage) {
		...
		if (worldClientIn != null) {
			if (!this.integratedServerIsRunning) {
				final AuthenticationService authenticationservice = new YggdrasilAuthenticationService(this.proxy,
						UUID.randomUUID().toString());
				final MinecraftSessionService minecraftsessionservice = authenticationservice
						.createMinecraftSessionService();
		...
		if (this.objectMouseOver != null && this.objectMouseOver.typeOfHit != RayTraceResult.Type.MISS) {
			...
			if (this.objectMouseOver.typeOfHit == RayTraceResult.Type.BLOCK) {
				...
				if (this.objectMouseOver.entityHit instanceof EntityPainting)
					...
					final ItemStack itemstack1 = entityitemframe.getDisplayedItem();
	...
	private void processKeyBinds() {
		...
		for (int i = 0; i < 9; ++i) {
			...
			final boolean flag1 = this.gameSettings.field_193630_aq.isKeyDown();
		...
		final boolean flag2 = this.gameSettings.chatVisibility != EntityPlayer.EnumChatVisibility.HIDDEN;
	...
	private boolean processKeyF3(final int p_184122_1_) {
		if (p_184122_1_ == 30) {
			...
			final boolean flag = this.debugRenderer.toggleDebugScreen();
	...
	private ByteBuffer readImageToBuffer(final InputStream imageStream) throws IOException {
		...
		final int[] aint = bufferedimage.getRGB(0, 0, bufferedimage.getWidth(), bufferedimage.getHeight(), (int[]) null,
					...
					if (this.objectMouseOver != null)
						...
						case BLOCK:
							...
							if (this.world.getBlockState(blockpos).getMaterial() != Material.AIR) {
								...
								final EnumActionResult enumactionresult = this.playerController.processRightClickBlock(
	...
	private void runGameLoop() throws IOException {
		...
		++this.fpsCounter;
		final boolean flag = this.isSingleplayer() && this.currentScreen != null
		...
		while (Minecraft.getSystemTime() >= this.debugUpdateTime + 1000L) {
			...
			this.debug = String.format("%d fps (%d chunk update%s) T: %s%s%s%s%s", Minecraft.debugFPS,
	...
	public void runTick() throws IOException {
		...
		this.mcProfiler.endSection();
		TaskManager manager = (TaskManager) Focess.getTaskManager();
		for (Task task : manager.getTasks())
			if (task.getPlugin().isEnable()) {
				task.reduce();
				if (task.getTime() == 0) {
					task.run();
		...
		for (Task task : manager.getTaskTimers())
			if (task.getPlugin().isEnable()) {
				task.reduce();
				if (task.getTime() == 0) {
					task.run();
	...
	private void runTickMouse() throws IOException {
		while (Mouse.next()) {
			...
			final long j = Minecraft.getSystemTime() - this.systemTime;
				...
				if (k != 0)
					if (this.player.isSpectator()) {
						...
						if (this.ingameGUI.getSpectatorGui().isMenuActive())
							...
							final float f = MathHelper.clamp(this.player.capabilities.getFlySpeed() + k * 0.005F, 0.0F,
									0.2F);
	...
	private void updateDisplayMode() throws LWJGLException {
		...
		if (!set.contains(displaymode) && Util.getOSType() == Util.EnumOS.OSX)
			...
			for (final DisplayMode displaymode1 : Minecraft.MAC_DISPLAY_MODES) {
				boolean flag = true;
